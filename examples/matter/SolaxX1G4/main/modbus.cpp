...
// Revision A Board. TX GPIO22, RX GPIO23, DE/RE GPIO18. 8N1, no flow control.
// Baud rate and slave address come from menuconfig (Application Configuration).

#define MODBUS_UART     UART_NUM_1
#define TXD_PIN         ((gpio_num_t)22)
#define RXD_PIN         ((gpio_num_t)23)
#define DE_RE_PIN       ((gpio_num_t)18)
#define MODBUS_SLAVE_ADDR CONFIG_MODBUS_SLAVE_ADDRESS
...
static void modbus_uart_configure(QueueHandle_t *event_queue)
{
    uart_config_t uart_config = {
        .baud_rate = CONFIG_MODBUS_BAUD_RATE,
...
esp_err_t modbus_read_input_registers(uint16_t start_addr, uint16_t count, uint16_t *out)
{
    ...
    ESP_LOGI(TAG, "Requesting %u regs from 0x%04X", count, start_addr);
    ...
    uint8_t response[5 + 2 * MODBUS_MAX_READ_REGISTERS];
    const int expected_len = 5 + 2 * count;

    // Read the three header bytes first so an exception response (which is
    // only five bytes long) doesn't leave us waiting out the full timeout.
    int len = uart_read_bytes(MODBUS_UART, response, 3, pdMS_TO_TICKS(MODBUS_RESPONSE_TIMEOUT_MS));
    if (len == 3)
    {
        int remaining = (response[1] & MODBUS_EXCEPTION_FLAG) ? 2 : expected_len - 3;
        int got = uart_read_bytes(MODBUS_UART, response + 3, remaining, pdMS_TO_TICKS(MODBUS_RESPONSE_TIMEOUT_MS));
        len += got > 0 ? got : 0;
    }
    ...
    uint16_t response_crc = crc16_modbus(response, len - 2);
    ...
    {
        ESP_LOGE(TAG, "Response from unexpected slave %u", response[0]);
        return ESP_FAIL;
    }

    if (response[1] == (FC_READ_INPUT_REGISTERS | MODBUS_EXCEPTION_FLAG))
    {
        ESP_LOGE(TAG, "Modbus exception 0x%02X", response[2]);
        return ESP_FAIL;
    }

    if (response[1] != FC_READ_INPUT_REGISTERS || len != expected_len || response[2] != 2 * count)
    {
        ESP_LOGE(TAG, "Malformed response (function 0x%02X, %d bytes)", response[1], len);
